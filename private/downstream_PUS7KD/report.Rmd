...
tx_list <- list.files(path = "/hpcnfs/scratch/FN/TL/cugolini/cov/analysis/PUS7_KD/map_to_recap_assembly/NANOCOMPORE/sampcomp/PUS7_KD_WT_upsampled/",pattern = "*_results.tsv" , full.names = TRUE,  recursive = T)
...
lapply(total_split,function(x){
  x %>%{
    ggplot(., aes(x=abs(Logit_LOR), y=-log10(GMM_logit_pvalue),  color=IUPAC)) +
      ...
      #ggrepel::geom_label_repel(data=filter(., (-log10(GMM_logit_pvalue)>=1 & abs(Logit_LOR)>0.5) ) ,aes(label=paste0(ref_kmer, " (",genomicPos,")")), colour="black", size=8) +
...
tx_list <- list.files(path = "/Volumes/scratch/FN/TL/cugolini/cov/analysis/PUS7_KD/map_to_recap_assembly/NANOCOMPORE/sampcomp/PUS7_KD_WT_upsampled/",pattern = "*_results.tsv" , full.names = TRUE,  recursive = T)
...
ggsave(p,file="/Volumes/scratch/FN/TL/cugolini/cov/scripts/downstream_PUS7KD/UGUAR.pdf", width = 15,height = 10)
...
final <- final %>%
  ...
  subset(final$GMM_logit_pvalue<=0.01)
...
p <- ggplot(final, aes(x=genomicPos, y=-log10(GMM_logit_pvalue), size=tx_cov, color=`Modification type`, shape=IVT)) +
      ...
      #ggrepel::geom_label_repel(data=filter(x, -log10(GMM_logit_pvalue)>=5), aes(label=paste0(ref_kmer, " (",genomicPos,")")), colour="black", size=5) +
      ggtitle("Modifications occurrence over all transcripts (GMM_pvalue<0.01) ") +
      theme_bw(28) +
    theme(plot.title = element_text(size = 30, face = "bold"),
            plot.subtitle = element_text(size = 28),
            legend.title=element_text(size=26),
            legend.text=element_text(size=24))
p
ggsave(p,file="/Volumes/scratch/FN/TL/cugolini/cov/scripts/downstream_PUS7KD/all_mods.pdf", width = 20,height = 20)
