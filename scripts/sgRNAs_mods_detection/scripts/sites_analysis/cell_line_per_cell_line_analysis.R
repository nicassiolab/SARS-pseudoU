...
data_per_line <- lapply(data_per_line, function(y){
  total_split <- split(y,y$ref_id)
...
for (i in unlist(cell_line)){
  tracks <- list.files(path = bdp(paste0("analysis/sgRNAs_mods_detection/",BASECALLING,"/",i,"/nanocompore/sampcomp/peakcalling_",which_nucl)),pattern = "*.bed" , full.names = TRUE,  recursive = T)
  tracks <- lapply(tracks, function(x){
    x <- read.table(x,col.names = c("ref_id","genomicPos","end","chr","score","strand","pos")) %>%
      ...
      rowwise() %>%
      mutate(fragment_ID = assign_fragment_kmer_first_nucl(genomicPos))           # assign RaPID fragment to each kmer
    x$fragment_ID <- factor(x$fragment_ID,
                            levels=c("No_Fragment",
                                     "Fragment1",
                                     "Fragment1_2",
                                     "Fragment2_3",
                                     "Fragment3_4",
  ...
  bed_list <- lapply(peakcalling_U, function(x){
    tx_id <- unique(x$id)
    x <- x %>%
      mutate(start=(genomicStartKmer)) %>%
      mutate(end=genomicEndKmer) %>%
      mutate(name=paste0("U_genomicStartKmer=",genomicStartKmer,"_genomicEndKmer_",genomicEndKmer,"_tx=",tx_id))%>%
      mutate(score=0)%>%
      mutate(strand="+") %>%
