...
blacklist_junctions <- junc_blacklist(unique(total_WT$ref_id))
total_WT <- subset(total_WT, !(genomicPos %in% blacklist_junctions))
...
total_WT_PUS7KD <- full_join(total,total_WT,by=c("pos","chr","ref_id","genomicPos","strand","ref_kmer","ORF","tx_length","IUPAC")) %>%
  subset(IUPAC== "UGUAR")
