...
assembly <-                                                                     # load Ugolini et al. NRCeq assembly file
  read.table(
    assembly_file,
    col.names = c(
      "chrom",
...
fragments_bed <- read_tsv(fragments_file, col_types = "cnn") %>%                # load genomic fragments bedfile
...
if(which_nucl=="U") {
  sitelist <- read_tsv(sitelist_pseudoU_file, col_types = "ccc") %>%
...
grna_sites <- total %>%
  subset(GMM_logit_pvalue<=pval_thresh & junctions=="No_junction" & abs(Logit_LOR)>=LOR_thresh) %>%
  ...
  total %>%
    subset(junctions=="No_junction") %>%
    {
      ggplot(., aes(x=abs(Logit_LOR), y=-log10(GMM_logit_pvalue),color=fragment_ID)) +
        geom_point(size=2) +
        {if (nrow(subset(total,(GMM_logit_pvalue<=pval_thresh & abs(Logit_LOR)>=LOR_thresh)))>0) ggrepel::geom_label_repel(data=dplyr::filter(., (GMM_logit_pvalue<=pval_thresh & abs(Logit_LOR)>=LOR_thresh & grepl("U",ref_kmer)==T)) ,aes(label=paste0(ref_kmer, " (",genomicPos,")")), colour="black", size=2)}+
...
write.table(grna_sites,file=rdp("WT_vs_IVT_sgRNAs_overlap_gRNAs.tsv"),sep = "\t", quote = F, col.names = T, row.names = F)
...
grna_bed <- grna_sites %>%
  subset(sgintersection==T) %>%
  dplyr::select(ref_id,genomicPos,ref_kmer)%>%
  mutate(ref_id="NC_045512.2",start=(genomicPos),end=(genomicPos+5),id=paste0("ref_kmer=",ref_kmer),score=0,strand="+")%>%
  dplyr::select(-ref_kmer,-genomicPos)

write.table(grna_bed,file=rdp("WT_vs_IVT_overlap_gRNAs_and_sgRNAs_sign_and_nojunc.bed"),sep = "\t", quote = F, col.names = F, row.names = F)
